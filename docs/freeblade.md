...
Play to find out what happens. Favor bold choices, dangerous situations, and meaningful consequences. The world is indifferent. Survival and legacy are earned through action.

## Core Rules

### Abilities

A character uses four Abilities:

- Strength: might, power, melee.
- Dexterity: finesse, agility, ranged.
- Presence: willpower, charm, spells.
- Constitution: fortitude, grit, resistance.

Assign +2, +1, 0, and -1 to your Abilities, or roll 2D6 for each using the list below. If no score is +1 or higher, reroll all Abilities.
...
Each session, a character has Focus equal to their Dexterity score, up to 3. Focus can be spent to reroll, make an enhanced attack (1D12 Damage), or make an incoming attack impaired (1D4 Damage).
...
Items: Most items take 1 Slot. Bulky items take 2. Petty items take none. Supplies such as torches or rations have limited uses which is tracked with a Step Die, which is rolled when the item is used or consumed. At D0, the item is depleted.
...
Criticals: When making a Check, a natural 20 is an automatic success while a natural 1 is an automatic failure regardless of Advantages or Disadvantages.

### Spells

Spells have limited uses which are tracked with a Step Die. Spells always take effect, but the caster must make a DR12 Presence Check or gain a "fatigued" Condition. After a cast, roll the spell Step Die. At D0, it is drained and cannot be used anymore. A character may refill a number of spells equal to their Presence score every time they take a Long Rest.
...
When an outcome is uncertain, the Referee may roll a 1D6 Die of Fate. On a 1 to 3, the answer is negative. On a 4 to 6, it is positive. The Referee may adjust the odds by assigning different X in 6 chances to the result depending on the situation.
...
Foraging: When foraging or hunting, roll the Die of Fate to determine if the characters find anything worthwhile. If so, increase a ration Step Die by a number of steps. The Referee may adjust the odds based on terrain and season.

Rations: Each character must consume one ration per day, rolling its Step Die, or gain a "starving" Condition.

Long Rest: Characters may spend one Watch (6 hours) taking a Long Rest to recover. If they do not do this at least once a day, they gain a "fatigued" Condition.
...
Light: A reliable light source is required while delving. Without one, characters have Disadvantage on all their actions and attacks. A single light source is usually enough for a group moving together. When igniting a new light, the player rolls the Step Die of the matching item. To track how long the light lasts, the Referee uses a duration Step Die: D4 for torches, D6 for lanterns. Every 20 minutes of real time, the Referee rolls that die and steps it down as normal. At D0, the light goes out and a new one must be lit.

Short Rest: Characters may spend one Turn (10 minutes) taking a Short Rest to recover. Resting may waste time or attract danger.
